...
class AddGistInFavoriteUseCaseTest {

    private val repository: GistRepository = mock {
        on { addGistInFavorite(DomainFactoryData.dummyGist) } doReturn Completable.complete()
    }
    private val postExecutionThread: PostExecutionThread = mock()
    private val useCase: AddGistInFavoriteUseCase =
        AddGistInFavoriteUseCase(repository, postExecutionThread)

    @Test
    fun `Check if add Gist in favorite is success`() {
